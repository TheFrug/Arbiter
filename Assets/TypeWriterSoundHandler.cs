...
public class TypewriterSoundHandler : ActionMarkupHandler
{
    ...
    {
        // Reset the timer when a line starts
        lastSoundTime = 0f;
    }
    ...
    {
        // 1. Check for blank space or null lines and skip
        if (index < 0 || index >= line.Text.Length)
        {
            return YarnTask.CompletedTask;
        }

        // 2. Suppress the burst effect by checking the time since the last sound played
        if (Time.time - lastSoundTime < soundCooldown)
        {
            // Too soon since the last character appeared; skip playing audio.
            return YarnTask.CompletedTask;
        }

        // 3. Do not play a sound on blank spaces
        ...
        // 4. Determine which sound key to use
        string soundToPlay = defaultSound;

        // Check if this is an internal voice (we now play the defaultSound rather than returning)
        if (!line.Text.Contains("[Fo]") && !line.Text.Contains("[In]") && !line.Text.Contains("[Em]"))
        {
            // It is not an internal voice, look up character mappings
        ...
        AudioClip clip = AudioManager.Instance.GetClipForTypewriter(soundToPlay);
